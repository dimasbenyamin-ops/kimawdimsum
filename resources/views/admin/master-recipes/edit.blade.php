...
@section('content')
    <div class="page-header">
        <div>
            <h1>✏️ Edit Master Resep</h1>
            <p>{{ $masterRecipe->name }}</p>
        </div>
        <a href="{{ route('admin.master-recipes.index') }}" class="btn btn-ghost">← Kembali</a>
    </div>

    <form method="POST" action="{{ route('admin.master-recipes.update', $masterRecipe) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $masterRecipe->name) }}"
            ...
            <div class="form-group">
                ...
                <input type="text" id="description" name="description" value="{{ old('description', $masterRecipe->description) }}"
        ...
        <div class="form-card">
            ...
            <div id="recipe-items-container">
                @forelse($masterRecipe->ingredients as $index => $item)
                    <div class="recipe-item" style="display:flex; gap:1rem; align-items:flex-end; margin-bottom: 1rem; background: var(--surface-hover); padding: 1rem; border-radius: var(--radius); border: 1px solid var(--border);">
                        <div style="flex: 2">
                            ...
                            <select name="ingredients[{{ $index }}][id]" class="ingredient-select" required style="width:100%">
                                <option value="">— Pilih Bahan —</option>
                                @foreach($ingredients as $ingredient)
                                    <option value="{{ $ingredient->id }}" data-unit="{{ $ingredient->unit->abbreviation }}" {{ $item->id == $ingredient->id ? 'selected' : '' }}>
                        ...
                        <div style="flex: 1">
                            ...
                            <div style="display:flex; align-items:center; gap:0.5rem">
                                <input type="number" name="ingredients[{{ $index }}][quantity]" value="{{ number_format($item->pivot->quantity, 4, '.', '') }}" step="any" min="0.0001" required style="width:100%">
                                <span class="unit-label" style="color:var(--muted); font-size:0.875rem; min-width:30px">{{ $item->unit->abbreviation }}</span>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            let itemIndex = {{ count($masterRecipe->ingredients) > 0 ? count($masterRecipe->ingredients) : 0 }};
